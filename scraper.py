...
def extract_links_fast(raw_html: str, base_url: str) -> list:
    ...
    priority_keywords = [
        "product", "solution", "service", "case-stud", "portfolio", "investment",
        "middle-market", "small-business", "private-equity", "private-debt",
        "infrastructure", "thermal", "cooling", "power", "manufacturing", "about"
    ]
...
def fetch_search_intelligence(company_name: str, domain: str) -> list:
    snippets = []
    queries = [
        f'"{company_name}" business model operations products solutions',
        f'"{company_name}" portfolio companies investments manufacturing facilities',
        f'"{company_name}" capital projects infrastructure expansion'
    ]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if abstract and len(abstract) > 50:
                    snippets.append(f"Profile ({heading}): {abstract}")
                for topic in data.get("RelatedTopics", [])[:3]:
                    if isinstance(topic, dict) and topic.get("Text"):
                        snippets.append(f"Fact: {topic.get('Text')}")
        except Exception:
            pass
...
def search_company_serp(query_or_url: str, api_key: str = None) -> dict:
    ...
    # 3. Google SERP API (if configured)
    serp_key = api_key or os.getenv("SERPAPI_API_KEY") or os.getenv("SERP_API_KEY") or ""
    if serp_key:
        try:
            params = {"engine": "google", "q": f'"{clean_name}" products solutions business overview', "api_key": serp_key, "num": 5}
            resp = requests.get("https://serpapi.com/search", params=params, timeout=6)
            if resp.status_code == 200:
                data = resp.json()
                serp_res = [f"Result: {r.get('title')} - {r.get('snippet')}" for r in data.get("organic_results", []) if r.get("snippet")]
                if serp_res:
